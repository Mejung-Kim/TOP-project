...
<script type="text/javascript">
	...
	$(function() {
		 ...
		 var list= new Array();
		  $("input[type='checkbox']:checked").each(function(i){
			...
			$("#addMag").click(function(){
					$.ajax({
						url:"insertMagazine.do",
						type:"post",
						dataType:"text",
						data:{"boardNoList":list},
		...
		$('#saveChanges').click(
				function() {
					...
					canvasDiv.style.width=parseInt(position[1]-position[0])+"px";
					canvasDiv.style.height=parseInt(position[3]-position[2])+"px";
		...
		$("#chooseBtn").click(function(){
			if($(this).val()=="Option On"){
				$("input[type=checkbox]").attr("style","display:block; margin-top:10px; width:17px; height:17px");				
		...
		$("#uploadImage").change(
						function() {
							...
							$.ajax({
									...
									cache : false,
									success : function(data) {
										$("#image").attr("src", sessionRealPath + "profileImage/${sessionScope.userNo}/"
	...
	function addItemToCanvas(arg_item, arg_x, arg_y, ev) {
		...
		console.log('cloneItem Size :: '+cloneItem.width+", "+cloneItem.height);
	...
	function reSizeImage(img, maxWidth, maxHeight) {
		...
		console.log('reSizeImage:: START width = '+img.width+", heihgt ="+img.height);
		...
		console.log('reSizeImage:: END width = '+img.width+", heihgt ="+img.height);
	}
	...
	//필요한 페이지 수 구하기(스크랩)
	function getNumOfGetScrap() {
		console.log('getNumOfGetScrap');
		$.ajax({
			url : 'getNumOfGetScrap.do', //데이터베이스에 아이템 저장
	...
	//페이지 불러오기(스크랩)
	function loadGetScrap() {
		$.ajax({
			url : 'loadGetScrap.do', //데이터베이스에 아이템 저장
			data : "&curPage=" + curPage.attr('name') + "&userNo=${sessionScope.userNo}",
			dataType : 'json',
			type : 'POST',
			async : false,
			success : function(data) {
				...
				$(data.resultBoardVoList).each(function(index, vo){ 
					...
					//디브 만들기
					var newScrapDiv = document.createElement('DIV');
					newScrapDiv.setAttribute('class','col-md-12 portfolio-item');
					newScrapDiv.setAttribute('id',new Date().getMilliseconds());
					var statement = "<h4 align='right' style='font-family:HY강M; font-size:20px;'>"+vo.title+"</h4><hr><a href='details.do?boardNo="+vo.boardNo+"'>"+ 
					...
					deleteDivIcon.setAttribute("class", "deleteDivIcon");
					deleteDivIcon.style.visibility = "hidden";
					newScrapDiv.appendChild(deleteDivIcon);
					
					newScrapDiv.addEventListener('mouseover', function(e) {
						deleteDivIcon.style.visibility = "visible";
					});
					
					newScrapDiv.addEventListener('mouseleave', function(e) {
						deleteDivIcon.style.visibility = "hidden";
					});
					...
					deleteDivIcon.addEventListener('click', function(e) {
						$.ajax({
							...
							success:function(data){
								...
								$('#'+newScrapDiv.getAttribute('id')).fadeOut(400, function() {
									$('#'+newScrapDiv.getAttribute('id')).remove();
								});				
							},
							error:function(err){
								alert('삭제에 실패했습니다.');
					...
					if((index%3)==0)
						document.getElementById('area_1').appendChild(newScrapDiv);
					else if((index%3)==1)
						document.getElementById('area_2').appendChild(newScrapDiv);
					else
						document.getElementById('area_3').appendChild(newScrapDiv);
	...
	function calMargin(){
		...
		while(curDiv.hasClass('canvasItemDiv')){
			var temp_right = parseInt(curDiv.position().left)+parseInt(curDiv.width());
			var temp_bottom = parseInt(curDiv.position().top)+parseInt(curDiv.height());
		...
		while(curDiv.hasClass('canvasItemDiv')){
			var temp_left = parseInt(parseInt(curDiv.position().left) - parseInt(position[0]));
			var temp_top = parseInt(parseInt(curDiv.position().top) - parseInt(position[2]));
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-lg-10">
				<h2 class="page-header">
					<small><a href="myPage.do">&nbsp;My Scraps</a></small>&nbsp;Get it Scraps
				</h2>
			...
			<div class="col-lg-2" align="center" style="position: fixed; right: 60px; bottom:80px;">
		...
		<div class="row">
			...
			<div class="col-lg-4 text-right">
				<a href=""><button class="btn btn-danger" id="addMag" type="button">Make
						a magazine</button></a>
			<!-- <button class="btn btn-danger" id="postScrap">Post Scrap</button> -->
			<button class="btn btn-danger" id="#">Post Scrap</button>
	...
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" style="width: 1100px">
			<div class="modal-content">
					...
					<div class="row">
						<div class="col-lg-4">
							<div style="border: #D9534F 1px solid; height: 480px">
								...
								<div id="closet" style="width: 334px; height: 396px; position: relative; display: none;">
									...
									<div class="row"
										...
										<input type="file" name="file" id="fileSelect"
											accept="image/*" class="btn btn-danger"
											style="position: absoulute;height:30px; width:300px;" multiple="">
									</div>
								</div>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-danger" id="saveChanges">Post</button>
					<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
				</div>
